...
using Samurai.Integration.Application.Actors.Nexaas;
...
namespace Samurai.Integration.NexaasWebJob
{
    public class Functions
    {
        ...
        [NoAutomaticTrigger]
        public async Task NexaasOrchestrator(
            ILogger logger, 
            CancellationToken cancellationToken)
        {
            _logger = logger;
            ActorSystem system = ActorSystem.Create("Nexaas", _configuration.GetSection("Akka")["Config"]);
            _webJobActor = system.ActorOf(NexaasWebJobActor.Props(_serviceProvider, cancellationToken));
            ...
            var _updateTenantQueue = _serviceBusService.GetQueueClient(TenantQueue.UpdateNexasWebjobQueue);
            _updateTenantQueue.RegisterMessageHandler(ProcessUpdateTenantMessageAsync, messageHandlerOptions);

            _logger.LogInformation("starting NexaasOrchestrator");
            while (!cancellationToken.IsCancellationRequested)
            {
                _logger.LogInformation("processing NexaasOrchestrator");

                try
                {
                    await _webJobActor.Ask(new OrchestrateNexaasMessage(), cancellationToken);
                }
                ...
                _logger.LogInformation("finishing NexaasOrchestrator");
            ...
            _logger.LogInformation("terminating NexaasOrchestrator");
            await system.Terminate();
            _logger.LogInformation("NexaasOrchestrator terminated");
        }
