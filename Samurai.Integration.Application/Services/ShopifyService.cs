...
using Samurai.Integration.Application.Strategy.NoteAttributes;
...
using Samurai.Integration.Domain.Models.GatewayNoteAttributes;
    ...
    public class ShopifyService
    {
                                                           ...
                                                           CancellationToken cancellationToken)
        {
            ...
            if (message.ProductInfo.ShopifyId.HasValue)
            {
                ...
                {
                    _logger.Warning($"ShopifyService - Error in UpdateFullProduct | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
            ...
            if (currentData == null)
            {
                ...
                if (message.ProductInfo.Status == true)
                {
                    ...
                    {
                        _logger.Warning($"ShopifyService - Error in UpdateFullProduct | {locationResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                        return new ReturnMessage { Result = Result.Error, Error = locationResult.Error };
                    }

                    List<Inputs.Variant> variantes = null;
                    ...
                    foreach (var activeSku in activeSkus)
                    {
                        ...
                        var variante = new Inputs.Variant
                        {
                            sku = activeSku.Sku,
                        ...
                        if (variantes == null)
                            variantes = new List<Inputs.Variant>();
                    ...
                    var createProductInput = new ProductCreateMutationInput
                    {
                        ...
                        {
                            published = message.ProductInfo.Status == null ? (bool?)null : message.ProductInfo.Status.Value && !shopifyData.SetProductsAsUnpublished,
                    ...
                    {
                        _logger.Warning($"ShopifyService - Error in UpdateFullProduct | {createResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
            ...
            else
            {
                ...
                {
                    var variants = new List<Inputs.Variant>();
                    var inventoryItemAdjustments = new List<Inputs.InventoryAdjustment>();
                    ...
                    foreach (var sku in activeSkus)
                    {
                        ...
                        {
                            //new sku
                            variants.Add(new Inputs.Variant
                            {
                                sku = sku.Sku,
                                ...
                                inventoryQuantities = new List<Inputs.InventoryQuantity>
                                    {
                                        new Inputs.InventoryQuantity
                                        {
                        ...
                        else
                        {
                            ...
                            inventoryItemAdjustments.Add(new Inputs.InventoryAdjustment
                            {
                    ...
                    while (optionPositionChanged != null)
                    {
                        ...
                        {
                            _logger.Warning($"ShopifyService - Error in UpdateFullProduct | {updateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                        ...
                        optionPositionChanged = optionPositionChanged == true ? (bool?)false : null;
                    }
                }
                else
                {
                    List<Inputs.Variant> variants = null;
                    ...
                    if (activeSkus.Any())
                    {
                        foreach (var activeSku in activeSkus)
                        {
                            ...
                            if (variants == null)
                                variants = new List<Inputs.Variant>();
        ...
        private Inputs.Product InputProduct(ShopifyDataMessage shopifyData,
                                            ProductResult currentData,
                                            Product.Info ProductInfo,
                                            List<Inputs.Variant> variants,
                                            bool? optionPositionChanged = false,
                                                              ...
                                                              CancellationToken cancellationToken)
        {
            ...
            if (message.ProductInfo.ShopifyId.HasValue)
            {
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdatePartialProduct | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
            ...
            if (currentData == null && !string.IsNullOrWhiteSpace(message.ProductInfo.ExternalId))
            {
                ...
                if (queryByTagResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdatePartialProduct | {queryByTagResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByTagResult.Error };
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (updateResult.Result == Result.Error)
                    {
                        _logger.Warning($"ShopifyService - Error in UpdatePartialProduct | {updateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                        return new ReturnMessage { Result = Result.Error, Error = updateResult.Error };
                    }
        ...
        private bool ProductVendorChanged(Product.Info productInfo, ProductResult currentData)
        {
        ...
        private bool ProductCategoriesChanged(ShopifyDataMessage shopifyData, Product.Info productInfo, ProductResult currentData)
        {
        ...
        private List<Inputs.Metafield> GetProductMetafields(ShopifyDataMessage shopifyData, Product.Info productInfo, ProductResult currentData = null)
        {
            ...
            if (productInfo.Metafields?.Any() == true)
            {
                foreach (var metafield in productInfo.Metafields)
                {
                    var shopifyMetafield = currentData?.metafields.edges
                                                    ...
                                                    .Select(x => new Inputs.Metafield { id = x.id }).FirstOrDefault() ?? new Inputs.Metafield();
        ...
        private string GetBody(Product.Info productInfo, ShopifyDataMessage shopifyData)
        {
        ...
        public async Task<ReturnMessage> UpdatePartialSku(ShopifyUpdatePartialSkuMessage message, ShopifyDataMessage shopifyData, QueueClient fullProductQueueClient, CancellationToken cancellationToken)
        {
            ...
            if (message.SkuInfo.ShopifyId.HasValue)
            {
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdatePartialSku | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
            ...
            if (currentData == null && !string.IsNullOrWhiteSpace(message.SkuInfo.Sku))
            {
                if (!shopifyData.CanMoveSku)
                {
                    ...
                    {
                        _logger.Warning($"ShopifyService - Error in UpdatePartialSku | {queryBySkuResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                ...
                else
                {
                    ...
                    {
                        _logger.Warning($"ShopifyService - Error in UpdatePartialSku | {queryProductsBySkuResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
            ...
            else
            {
                if (message.SkuInfo.Status == true)
                {
                    ...
                    if (createResult.Result == Result.Error)
                    {
                        _logger.Warning($"ShopifyService - Error in UpdatePartialSku | {createResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                        return new ReturnMessage { Result = Result.Error, Error = createResult.Error };
                    }
        ...
        public async Task<ReturnMessage> UpdatePrice(ShopifyUpdatePriceMessage message, ShopifyDataMessage shopifyData, QueueClient fullProductQueueClient, CancellationToken cancellationToken)
        {
            ...
            if (message.Value.ShopifyId.HasValue)
            {
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdatePrice | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
            ...
            if (currentData == null && !string.IsNullOrWhiteSpace(message.Value.Sku))
            {
                ...
                if (queryBySkuResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdatePrice | {queryBySkuResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryBySkuResult.Error };
                }
            ...
            else
            {
                ...
                if (createResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdatePrice | {createResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = createResult.Error };
                }
        ...
        public async Task<ReturnMessage> UpdateStock(ShopifyUpdateStockMessage message, ShopifyDataMessage shopifyData, QueueClient fullProductQueueClient, QueueClient updateStockKitQueue, CancellationToken cancellationToken)
        {
            ...
            if (message.Value.ShopifyId.HasValue)
            {
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    ...
                    _logger.Warning($"ShopifyService - Error in UpdateStock | {errorMessage.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData,
            ...
            else
            {
                ...
                if (_shopifyData.EnabledMultiLocation)
                {
                    ...
                    {
                        _logger.Warning($"ShopifyService - Error in UpdateStock", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData,
                    ...
                    if (locationsShop.Result == Result.Error)
                    {
                        ...
                        _logger.Warning($"ShopifyService - Error in UpdateStock | {locationsShop.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData,
                    ...
                    if (createNewLocation)
                    {
                        ...
                        if (createResult.Result == Result.Error)
                        {
                            ...
                            _logger.Warning($"ShopifyService - Error in UpdateStock | {createResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData,
                ...
                if (message.Value.Quantity != inventoryItem.available)
                {
                    var createResult = await _apiActorGroup.Ask<ReturnMessage<InventoryUpdateMutationOutput>>(
                           ...
                           {
                               input = new Inputs.InventoryLevel
                               {
                                                        ...
                                                        CancellationToken cancellationToken)
        {
            ...
            if (queryByIdResult.Result == Result.Error)
            {
                _logger.Warning($"ShopifyService - Error in UpdateStockKit | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
            }
            ...
            var updateResult = await _apiActorGroup.Ask<ReturnMessage<ProductAndInventoryUpdateMutationOutput>>(
                    new ProductAndInventoryUpdateMutation(new ProductAndInventoryUpdateMutationInput
                    {
                        ...
                        inventoryItemAdjustments = new List<Inputs.InventoryAdjustment> {
                            new Inputs.InventoryAdjustment {
            ...
            if (updateResult.Result == Result.Error)
            {
                _logger.Warning($"ShopifyService - Error in UpdateStockKit | {updateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), new ProductAndInventoryUpdateMutationInput
                {
                    input = new Inputs.Product
                    {
                        id = currentData.id,
                                                             ...
                                                             CancellationToken cancellationToken)
        {
            ...
            if (!string.IsNullOrEmpty(message.ExternalProductId))
            {
                ...
                if (queryByTagResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdateProductImages | {queryByTagResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByTagResult.Error };
                }
            ...
            if (currentData == null && message.ShopifyProductId.HasValue)
            {
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdateProductImages | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
            ...
            var clearResult = await _apiActorGroup.Ask<ReturnMessage<ProductUpdateMutationOutput>>(
                        new ProductUpdateMutation(new ProductUpdateMutationInput
                        {
                            ...
                            {
                                id = currentData.id,
                                images = new List<Inputs.Image>() //clean all images
                            }
                        }), cancellationToken);

            if (clearResult.Result == Result.Error)
            {
                _logger.Warning($"ShopifyService - Error in UpdateProductImages | {clearResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), new ProductUpdateMutationInput
                {
                    input = new Inputs.Product
                    {
                        id = currentData.id,
            ...
            foreach (var images in message.Images.ImageUrls.Chunk(5))
            {
                var appendResult = await _apiActorGroup.Ask<ReturnMessage<ProductAppendImagesMutationOutput>>(
                           ...
                           {
                               input = new Inputs.ProductAppendImage
                               {
                                   id = currentData.id,
                                   images = images.Select(i => new Inputs.Image
                                   {
                                       src = i
                                   }).ToList()
                               }
                ...
                {
                    _logger.Warning($"ShopifyService - Error in UpdateProductImages | {appendResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), new ProductAppendImagesMutationInput
                    {
                        input = new Inputs.ProductAppendImage
                        {
                            id = currentData.id,
            ...
            var updateResult = await _apiActorGroup.Ask<ReturnMessage<ProductUpdateMutationOutput>>(
                        new ProductUpdateMutation(new ProductUpdateMutationInput
                        {
                            ...
                            {
                                id = currentData.id,
                                variants = currentData.variants.edges.Select(v => new Inputs.Variant
                                {
            ...
            if (updateResult.Result == Result.Error)
            {
                _logger.Warning($"ShopifyService - Error in UpdateProductImages | {updateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), new ProductUpdateMutationInput
                {
                    input = new Inputs.Product
                    {
                        id = currentData.id,
                                                   ...
                                                   CancellationToken cancellationToken)
        {
            ...
            if(message.ShopifyId != 5334490153203)
            {
            ...
            try
            {
                var queryByIdResult = await _apiActorGroup.Ask<ReturnMessage<REST.OrderResult>>(
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    ...
                    _logger.Warning($"ShopifyService - Error in ListOrder | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
                ...
                if (shopifyData.Id != Tenants.PitBull && queryByIdResult.Data.order.gateway != "mercado_pago" && (queryByIdResult.Data.order.note_attributes is null || queryByIdResult.Data.order.note_attributes.Count <= 0))
                {
                    try
                    {
                        queryByIdResult.Data.order.note_attributes = await GetNoteAttributes(queryByIdResult.Data.order.gateway, queryByIdResult.Data.order.checkout_token);
                                                                            ...
                                                                            CancellationToken cancellationToken)
        {
            ...
            if (currentData.gateway == "mercado_pago")
            {
                ...
                return new ShopifySendOrderToERPMessage
                {
                    ...
                    Approved = currentData.financial_status == REST.OrderResult.FinancialStatus.authorized || currentData.financial_status == REST.OrderResult.FinancialStatus.paid,
                    Shipped = currentData.fulfillment_status == REST.OrderResult.OrderFulfillmentStatus.fulfilled,
                    TrackingNumber = currentData.fulfillments?.FirstOrDefault()?.tracking_number,
                    Delivered = currentData.fulfillments?.FirstOrDefault()?.shipment_status == REST.OrderResult.ShipmentStatus.delivered,
        ...
        private OrderNoteAttributesModelMercadoPagoInfo GetNoteAttributesMercadoPago(Order currentData)
        {
        ...
        private (string, string) GetCustomerName(Order currentOrder)
        {
        ...
        private OrderNoteAttributesModelInfo GetNoteAttributes(REST.OrderResult.Order currentData)
        {
        ...
        private bool IsGift(REST.OrderResult.LineItem item)
        {
            return item.properties?.Any(w => w.name == "Brinde" && w.value == "true") ?? false;
        }

        private string GetPaymentType(Order currentData)
        {
            var payment = string.Empty;
            if (currentData.gateway == "mercado_pago")
            {
                return currentData.note_attributes.FirstOrDefault(n => n.name == "payment_type")?.value ?? currentData.gateway;
            }

            payment = currentData.note_attributes.FirstOrDefault(n => n.name == "shipping_payment_type")?.value ?? currentData.gateway;
        ...
        private ShopifySendOrderAddressToERPMessage GetAddressDataMercadoPago(ShopifyDataMessage shopifyData,
                                                                              REST.OrderResult.Address address,
                                                                              ...
                                                                              bool isBillingAddress)
        {
            ...
            string[] parts = null;
            char[] delimiters = { ',', '-' };
            var virgulaHifen = @"^[\w\s]+,\s*[\w\s]+-\s*[\w\s]+$";
            ...
            string getAddress(REST.OrderResult.Address address, bool isBillingAddress, OrderNoteAttributesModelMercadoPagoInfo notesAddress, ShopifyDataMessage shopifyData)
            {
                if (isBillingAddress)
            ...
            string getComplement(REST.OrderResult.Address address, OrderNoteAttributesModelMercadoPagoInfo notesAddress, bool isBillingAddress)
            {
                if (isBillingAddress)
            ...
            string getNumber(ShopifyDataMessage shopifyData, REST.OrderResult.Address address, OrderNoteAttributesModelMercadoPagoInfo notesAddress, bool isBillingAddress)
            {
                if (isBillingAddress)
        ...
        private ShopifySendOrderAddressToERPMessage GetAddressData(ShopifyDataMessage shopifyData, REST.OrderResult.Address address, ShopifySendOrderAddressToERPMessage notesAddress)
        {
            var contactName = $"{address.first_name} {address.last_name}";
            if (!string.IsNullOrWhiteSpace(contactName) && contactName.Length > 50)
                contactName = contactName[0..50];
        ...
        public async Task<ReturnMessage> UpdateOrderTagNumber(ShopifyUpdateOrderTagNumberMessage message, ShopifyDataMessage shopifyData, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                new OrderByIdQuery(message.ShopifyId, OrderResult.Tags), cancellationToken);
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    ...
                    _logger.Warning($"ShopifyService - Error in UpdateOrderTagNumber | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
        ...
        public async Task<ReturnMessage> UpdateOrderStatus(ShopifyUpdateOrderStatusMessage message, ShopifyDataMessage shopifyData, CancellationToken cancellationToken)
        {
            ...
            {
                var queryByIdResult = await _apiActorGroup.Ask<ReturnMessage<OrderByIdQueryOutput>>(
                       new OrderByIdQuery(message.ShopifyId.Value, OrderResult.Status), cancellationToken
                   );

                if (queryByIdResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
            ...
            if (currentData == null && !string.IsNullOrWhiteSpace(message.OrderExternalId))
            {
                var queryByTagResult = await _apiActorGroup.Ask<ReturnMessage<OrderByTagQueryOutput>>(
                    new OrderByTagQuery(SetTagValue(Tags.OrderExternalId, message.OrderExternalId), OrderResult.Status), cancellationToken
                );

                if (queryByTagResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {queryByTagResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByTagResult.Error };
                }
                ...
                if (message.Cancellation.IsCancelled)
                {
                    if (currentData.cancelledAt == null)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {orderCancelResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    ...
                    else
                    {
                        if (message.Payment.IsPaid)
                        {
                            if (currentData.displayFinancialStatus == "PENDING")
                            {
                                ...
                                {
                                    _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {markAsPaidResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                        ...
                        if (message.Shipping.IsShipped)
                        {
                            if (currentData.displayFulfillmentStatus == "UNFULFILLED")
                            {
                                ...
                                {
                                    _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {fulfillmentCreateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                            ...
                            else
                            {
                                if (!message.Shipping.IsDelivered)
                                {
                                    ...
                                    if (currentTracking == null || currentTracking.number != message.Shipping.TrackingObject)
                                    {
                                        ...
                                        {
                                            _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {fulfillmentTrackingInfoUpdateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                        ...
                        if (message.Shipping.IsDelivered)
                        {
                            if (currentFulfillment?.id != null && currentFulfillment?.deliveredAt == null)
                            {
                                ...
                                {
                                    _logger.Warning($"ShopifyService - Error in UpdateOrderStatus | {fulfillmentEventCreateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
        ...
        public async Task<ReturnMessage> UpdateTrackingOrder(ShopifyUpdateTrackingOrder message, ShopifyDataMessage shopifyData, CancellationToken cancellationToken)
        {
            ...
            if (message.ShopifyId.HasValue)
            {
                ...
                if (queryByIdResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdateTrackingOrder | {queryByIdResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByIdResult.Error };
                }
            ...
            if (currentData == null && !string.IsNullOrWhiteSpace(message.OrderExternalId))
            {
                ...
                if (queryByTagResult.Result == Result.Error)
                {
                    _logger.Warning($"ShopifyService - Error in UpdateTrackingOrder | {queryByTagResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                    return new ReturnMessage { Result = Result.Error, Error = queryByTagResult.Error };
                }
                ...
                else
                {
                    ...
                    if (message.Shipping.IsShipped)
                    {
                        if (currentData.displayFulfillmentStatus == "UNFULFILLED")
                        {
                            ...
                            if (fulfillmentCreateResult.Result == Result.Error)
                            {
                                _logger.Warning($"ShopifyService - Error in UpdateTrackingOrder | {fulfillmentCreateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                                return new ReturnMessage { Result = Result.Error, Error = fulfillmentCreateResult.Error };
                            }
                        ...
                        else
                        {
                            if (!message.Shipping.IsDelivered || (message.Shipping.IsDelivered && currentFulfillment?.trackingInfo?.FirstOrDefault() == null))
                            {
                                ...
                                if (currentTracking == null || currentTracking.number != message.Shipping.TrackingCode)
                                {
                                    ...
                                    if (fulfillmentTrackingInfoUpdateResult.Result == Result.Error)
                                    {
                                        _logger.Warning($"ShopifyService - Error in UpdateTrackingOrder | {fulfillmentTrackingInfoUpdateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                                        return new ReturnMessage { Result = Result.Error, Error = fulfillmentTrackingInfoUpdateResult.Error };
                                    }
                    ...
                    if (message.Shipping.IsDelivered)
                    {
                        if (currentFulfillment?.id != null && currentFulfillment?.deliveredAt == null)
                        {
                            ...
                            if (fulfillmentEventCreateResult.Result == Result.Error)
                            {
                                _logger.Warning($"ShopifyService - Error in UpdateTrackingOrder | {fulfillmentEventCreateResult.Error.Message}", LoggingExtensions.FromService(LoggingExtensions.GetCurrentMethod(), message, shopifyData));
                                return new ReturnMessage { Result = Result.Error, Error = fulfillmentEventCreateResult.Error };
                            }
                        }
                    }
        ...
        public bool HasToNotifyFulfillment(OrderResult order, ShopifyDataMessage shopifyData)
        {
        ...
        public string GetExternalOrderNumber(Tenant tenant, string orderNumber)
        {
            return tenant.Type switch
            {
                ...
                TenantType.Nexaas => string.Concat(tenant.NexaasData.OrderPrefix, orderNumber.TrimStart('#')),
        ...
        private List<Inputs.Metafield> MergeMetafields(List<Inputs.Metafield> update, Connection<MetafieldResult> current)
        {
        ...
        private List<Inputs.InventoryQuantity> FillInventory(Product.SkuInfo sku, List<Edges<LocationResult>> shopifyLocations)
        {
            var inventories = new List<Inputs.InventoryQuantity>();
            ...
            if (_shopifyData.EnabledMultiLocation && sku.Stock.Locations.Any())
            {
                foreach (var warehouse in sku.Stock.Locations)
                {
                    ...
                    inventories.Add(new Inputs.InventoryQuantity
                    {
        ...
        private string FillLocation(Product.SkuInfo sku, string locationId)
        {
        ...
        public async Task<bool> ValidAndReturnProductKit(REST.OrderResult.Order order, ShopifyDataMessage shopifyData, QueueClient updateOrderTagNumber, QueueClient updateStockQueueClient, CancellationToken cancellationToken = default)
        {
            var itemsToRemove = new List<long>();
            var itemsToInsert = new List<REST.OrderResult.LineItem>();

            (REST.OrderResult.Order order, ProductResult product) orderItem = (order, null);
            foreach (var item in order.line_items)
            {
                ...
                if (isProductKit)
                {
                    ...
                    foreach (var sku in skus)
                    {
                        ...
                        itemsToInsert.Add(new REST.OrderResult.LineItem
                        {
                            ...
                            discount_allocations = new List<REST.OrderResult.DiscountAllocation> { new REST.OrderResult.DiscountAllocation { amount = newDiscount } }
                        });
        ...
        public async Task ProcessStockProductKit(ShopifyDataMessage shopifyData, (REST.OrderResult.Order order, ProductResult product) order, QueueClient updateOrderTagNumber, QueueClient updateStockQueueClient, CancellationToken cancellationToken = default)
        {
        ...
        private List<string> FillProductTags(ShopifyDataMessage shopifyData, Product.Info productInfo, List<string> currentTags = null)
        {
            ...
            Action<List<Product.Category>, string> fillCategoryTags = null;
        ...
        private void FillKitTags(List<Product.SkuKit> skuKits, ref List<string> result, ref List<string> tagIds)
        {
        ...
        private async Task FillMultiLocation(long orderId, Order orderToUpdate, CancellationToken cancellationToken)
        {
            if (_shopifyData.EnabledMultiLocation)
            {
                var orderFulfillment = await _apiActorGroup.Ask<ReturnMessage<REST.OrderResult>>(
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var versionShopify = tenant.Id == 57 ?
                ...
                var queryByDateResult = await client.Post(
                    new OrderByDateQuery(beginDate, utcNow, null, OrderResult.Tags, filters: additionalFilters),
                    cancellationToken);
                ...
                while (queryByDateResult.orders.pageInfo.hasNextPage == true && !cancellationToken.IsCancellationRequested)
                {
                    queryByDateResult = await client.Post(
                        new OrderByDateQuery(beginDate, DateTime.UtcNow, queryByDateResult.orders.edges.Last().cursor, OrderResult.Tags, filters: additionalFilters),
                    cancellationToken);
