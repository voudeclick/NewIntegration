...
{
    public abstract class BaseNexaasTenantActor : ReceiveActor
    {
        public readonly string _actorName;
        protected readonly ILoggingAdapter _log;
        protected NexaasData _nexaasData;

        public BaseNexaasTenantActor(string actorName)
        {
        ...
        {
            if (_tenantLogger == null || _tenantLogger.Value.StoreHandle != _nexaasData.StoreHandle)
                _tenantLogger = (_nexaasData.StoreHandle, _log.ForContext("StoreHandle", _nexaasData.StoreHandle));
